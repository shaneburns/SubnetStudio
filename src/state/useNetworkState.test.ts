...
describe('automation state application', () => {
  ...
  it('rejects equal-split prefixes that would exceed the block materialization limit', () => {
    const initial: NetworkState = {
      ...makeBaseState(),
      base: { ip: '0.0.0.0', prefix: 0 },
    };
    const plan: AutomationPlan = {
    ...
    expect(result.ok).toBe(false);
    expect(result.applied).toBe(0);
    expect(result.message).toMatch(/exceeding the limit/i);
    expect(result.state).toEqual(initial);
  });

  it('clears a preserved equal-split prefix when a base change would exceed the block limit', () => {
    const initial: NetworkState = {
      ...makeBaseState(),
      equalSplitPrefix: 32,
    };
    const plan: AutomationPlan = {
      summary: 'set base network to 0.0.0.0/0',
      confidence: 0.95,
      requiresConfirmation: true,
      actions: [{ intent: 'set-base-cidr', cidr: '0.0.0.0/0' }],
    };

    const result = applyAutomationPlanToState(initial, plan);
    expect(result.ok).toBe(true);
    expect(result.state.base).toEqual({ ip: '0.0.0.0', prefix: 0 });
    expect(result.state.equalSplitPrefix).toBeNull();
  });
